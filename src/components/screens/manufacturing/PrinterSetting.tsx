...
const PrinterSetting: React.FC<PrinterSettingProps> = ({ onBack }) => {
  ...
  const handleSave = async () => {
    ...
    if (printer.enabled && (printer.tokenCopies < 1 || printer.tokenCopies > 10)) {
      toast({
        title: "Validation Error",
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-medium text-foreground">Printer</h3>
                <p className="text-sm text-muted-foreground">Enable or disable printer</p>
              </div>
            </div>
            <Switch
              checked={printer.enabled}
              onCheckedChange={(checked) => setPrinter(prev => ({ ...prev, enabled: checked }))}
            />
          </div>
        </Card>
